...
@st.cache_data(ttl=300)
def load_consumabili_data(impianto_scelto):
    """Carica i log operativi dei chimici da Supabase."""
    try:
        from supabase import create_client
        supabase = create_client(st.secrets["SUPABASE_URL"], st.secrets["SUPABASE_KEY"])
        nome_impianto = "kaktus" if "Kaktus" in impianto_scelto else "pingwe"
        res = supabase.table(f"{nome_impianto}_consumabili").select("*").order("data_registrazione", desc=True).execute()
        df = pd.DataFrame(res.data)
        if not df.empty:
