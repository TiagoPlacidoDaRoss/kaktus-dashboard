...
def genera_report_pdf(impianto_scelto, config_attuale, start_date, end_date, df_ro_raw, df_uf, df_nas, selected_sections, selected_series, include_notes=True):
    ...
    def header_footer(canvas, document):
        ...
        # 1. Disegna la linea di separazione grigio chiara
        canvas.setStrokeColor(colors.HexColor("#CBD5E1"))
        canvas.line(document.leftMargin, 1.1 * cm, A4[0] - document.rightMargin, 1.1 * cm)
        
        # 2. Inserimento del Logo (se il file esiste nella cartella)
        text_x = document.leftMargin
        logo_path = "Logo v1.png"
        import os
        if os.path.exists(logo_path):
            # Posiziona il logo a 0.4cm dal fondo, forzando un'altezza massima e mantenendo le proporzioni.
            # mask='auto' assicura che lo sfondo trasparente del PNG venga rispettato.
            canvas.drawImage(logo_path, document.leftMargin, 0.4 * cm, width=2.5 * cm, height=0.55 * cm, preserveAspectRatio=True, mask='auto')
            text_x += 2.8 * cm  # Sposta il testo del copyright a destra del logo
            
        # 3. Testo del Footer (Impianto + Copyright)
        canvas.setFont(regular_font, 7)
        canvas.setFillColor(colors.HexColor("#64748B"))
        
        footer_text = f"{plant_display}  |  " + _r(
            "Generato da monitoraimpianti.it - Tutti i diritti riservati",
            "Generated by monitoraimpianti.it - All rights reserved"
        )
        canvas.drawString(text_x, 0.55 * cm, footer_text)
        
        # 4. Numero di Pagina (allineato a destra)
        canvas.drawRightString(A4[0] - document.rightMargin, 0.55 * cm, f"{_r('Pagina', 'Page')} {canvas.getPageNumber()}")
...
def render_consumabili(impianto_scelto):
    st.header(ui_text("🛢️ Registro e Scorte Consumabili", "🛢️ Consumables Log & Stock"))
    
    # Rilevamento dinamico dell'impianto per supportare sia Pingwe che Kaktus
    nome_impianto = "kaktus" if "Kaktus" in impianto_scelto else "pingwe"
    tabella_db = f"{nome_impianto}_consumabili"
    
    try:
        from supabase import create_client
        supabase = create_client(st.secrets["SUPABASE_URL"], st.secrets["SUPABASE_KEY"])
        res = supabase.table(tabella_db).select("*").order("data_registrazione", desc=True).execute()
        df_cons = pd.DataFrame(res.data)
    except Exception as e:
        st.info(ui_text(
            f"Il modulo Consumabili non è ancora configurato su Supabase per {impianto_scelto}.", 
            f"The Consumables module is not yet configured in Supabase for {impianto_scelto}."
        ))
        return

    if df_cons.empty:
        st.info(ui_text(
            f"Nessun dato consumabili trovato per l'impianto {impianto_scelto}.", 
            f"No consumables data found for plant {impianto_scelto}."
        ))
        return

    df_cons['data_registrazione'] = pd.to_datetime(df_cons['data_registrazione'])
    
    # ==========================================
    # 1. CRUSCOTTO GIACENZE 
    # ==========================================
    st.subheader(ui_text("Cruscotto Giacenze Attuali", "Current Stock Dashboard"))
    
    chimici = df_cons['tipo_chimico'].unique()
    cols = st.columns(max(3, len(chimici)))
    
    for i, chimico in enumerate(chimici):
        df_chimico = df_cons[df_cons['tipo_chimico'] == chimico]
        ultima_reg = df_chimico.iloc[0] # Il df è già ordinato per data decrescente
        
        nome_chimico_disp = str(chimico).capitalize()
        if chimico == 'soda': nome_chimico_disp = "Soda Caustica"
        
        with cols[i]:
            st.metric(
                label=nome_chimico_disp, 
                value=f"{ultima_reg['remaining_kg']:.2f} Kg",
                delta=ui_text(f"Ultimo rabbocco: {ultima_reg['refill_kg']:.2f} Kg", f"Last refill: {ultima_reg['refill_kg']:.2f} Kg"),
                delta_color="off"
            )
            st.caption(ui_text(f"Aggiornato il: {ultima_reg['data_registrazione'].strftime('%d/%m/%Y')}", f"Updated on: {ultima_reg['data_registrazione'].strftime('%d/%m/%Y')}"))
    ...
    # 2. GRAFICO TREND
    # ==========================================
    st.subheader(ui_text("📈 Trend Consumi e Rifornimenti", "📈 Consumption & Refill Trend"))
    
    chimico_sel = st.selectbox(
        ui_text("Seleziona Prodotto Chimico:", "Select Chemical Product:"), 
        chimici, 
        format_func=lambda x: str(x).capitalize() if x != 'soda' else 'Soda Caustica'
    )
    
    df_plot = df_cons[df_cons['tipo_chimico'] == chimico_sel].sort_values('data_registrazione')
    
    if not df_plot.empty:
        fig = go.Figure()
        
        # Consumo (Barre Arancioni)
        fig.add_trace(go.Bar(
            x=df_plot['data_registrazione'], y=df_plot['refill_kg'], 
            name=ui_text('Aggiunto al Tank (Kg)', 'Added to Tank (Kg)'), 
            marker_color='#F39C12', yaxis='y2', opacity=0.7
        ))
        
        # Rifornimenti di Magazzino (Barre Verdi)
        fig.add_trace(go.Bar(
            x=df_plot['data_registrazione'], y=df_plot['intake_kg'], 
            name=ui_text('Nuovo Carico Arrivato (Kg)', 'New Intake (Kg)'), 
            marker_color='#27AE60', yaxis='y2', opacity=0.7
        ))
        
        # Linea della Giacenza totale
        fig.add_trace(go.Scatter(
            x=df_plot['data_registrazione'], y=df_plot['remaining_kg'], 
            mode='lines+markers', name=ui_text('Giacenza Totale (Kg)', 'Total Stock (Kg)'), 
            line=dict(color='#0F4C5C', width=3)
        ))
        
        fig.update_layout(
            yaxis=dict(title=ui_text('Giacenza (Kg)', 'Stock (Kg)')),
            yaxis2=dict(title=ui_text('Movimentazione (Kg)', 'Movement (Kg)'), overlaying='y', side='right', showgrid=False),
            hovermode='x unified',
            barmode='group',
    ...
    # ==========================================
    # 3. TABELLA DETTAGLIO STORICO
    # ==========================================
    st.subheader(ui_text("Dettaglio Operazioni", "Operations Detail"))
    
    df_display = df_cons.copy()
    df_display['data_registrazione'] = df_display['data_registrazione'].dt.strftime('%d/%m/%Y')
    
    colonne_da_mostrare = ['data_registrazione', 'tipo_chimico', 'tank_level_before', 'refill_kg', 'tank_level_after', 'intake_kg', 'remaining_kg']
    if 'firma_operatore' in df_display.columns:
        df_display['firma'] = df_display['firma_operatore'].apply(
            lambda x: ui_text("✅ Presente", "✅ Present") if pd.notna(x) and x != 'N/A' and x != '' else ui_text("❌ Assente", "❌ Missing")
        )
        colonne_da_mostrare.append('firma')
        
    df_display = df_display[colonne_da_mostrare]
    
    mappa_colonne = {
        'data_registrazione': ui_text('Data', 'Date'),
        'tipo_chimico': ui_text('Prodotto', 'Chemical'),
        'tank_level_before': ui_text('Livello Prima (L)', 'Level Before (L)'),
        'refill_kg': ui_text('Aggiunto (Kg)', 'Added (Kg)'),
        'tank_level_after': ui_text('Livello Dopo (L)', 'Level After (L)'),
        'intake_kg': ui_text('Nuovo Carico (Kg)', 'New Intake (Kg)'),
        'remaining_kg': ui_text('Giacenza (Kg)', 'Remaining Stock (Kg)'),
        'firma': ui_text('Firma Operatore', 'Operator Signature')
    }
    
    df_display.rename(columns=mappa_colonne, inplace=True)
    
    col1, col2 = st.columns([8, 2])
    with col2:
        st.download_button(
            label="📥 Esporta CSV" if UI_LANGUAGE == "it" else "📥 Export CSV", 
            data=converti_df_csv(df_display), 
            file_name=f'storico_consumabili_{nome_impianto}.csv', 
            mime='text/csv'
        )
...
if __name__ == '__main__':
    ...
    if impianto_scelto == FLEET_OVERVIEW_KEY:
        ...
        menu_opzioni = ["🔵 Osmosi Inversa (RO)", "⚡ Inverter & Pompe", "📈 Grafici Personalizzati", 
                        "🔮 Manutenzione Predittiva", "⚖️ Confronto Periodi", "📊 Produzione & ATM", "💧 Qualità Acqua (Manuale)","🛢️ Registro Consumabili", "📄 Report"]
        if config_attuale["has_uf"]: 
        ...
        elif sezione_selezionata == "🛢️ Registro Consumabili":
            render_consumabili(impianto_scelto)
